...
protected:
public:
	Missile(unsigned int _id, unsigned int _customId, unsigned int _x, unsigned int _y, unsigned int _dir, unsigned int _damage, char _mapSymbol, wchar_t _screenSymbol, unsigned int _color, unsigned int _serialNum) : GameObject(_id, _customId, _x, _y, _dir, _mapSymbol, _screenSymbol, _color) {
		damage = _damage;
